...
def test_build_options_disallowed_tools_matches_guardrail_policy():
    ...
    # workspace_notes, memory_candidates, fetch_holdings, and identity_check
    # are always added (unlike skill_scripts, which is conditional) — see
    # cas._build_options — so the guardrail's own denied server list must
    # include all four too. get_holdings itself is unconditionally
    ...
    expected = tools.guardrail.denied_tool_names(
        [*FAKE_MCP_SERVERS.keys(), "workspace_notes", "memory_candidates", "fetch_holdings", "identity_check"]
    ) | {"mcp__kite_gateway__get_holdings"}
    assert set(options.disallowed_tools) == expected
    assert len(expected) == 6 * len(ORDER_TOOL_NAMES) + 1
